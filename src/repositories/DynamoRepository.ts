...
export enum LSI {
    FILTER1_INDEX = 'FILTER1_INDEX',
    FILTER2_INDEX = 'FILTER2_INDEX',
    FILTER3_INDEX = 'FILTER3_INDEX',
}
...
export class DynamoRepository<T extends DynamoModel> {
    ...
    public async increment(lookup_key: string, reference_key: string, column: string, amount: number): Promise<void> {
        const params: DocumentClient.UpdateItemInput = {
            ...
            UpdateExpression: `SET ${column} = ${column} + :amount`,
            ExpressionAttributeValues: {
                ':amount': amount,
            },
    ...
    public async decrement(lookup_key: string, reference_key: string, column: string, amount: number): Promise<void> {
        const params: DocumentClient.UpdateItemInput = {
            TableName: TABLE_NAME,
            Key: {
                lookup_key,
    ...
    public async columnUpdate(lookup_key: string, reference_key: string, column: string, data: unknown): Promise<void> {
        const params: DocumentClient.UpdateItemInput = {
            TableName: TABLE_NAME,
            Key: {
                lookup_key,
            ...
            UpdateExpression: `SET ${column} = :data`,
            ExpressionAttributeValues: {
                ':data': data,
            },
            ReturnValues: 'ALL_NEW',
